...
metaagent_prompt = lambda user_request, metadata_text, current_plan=None, current_code=None, history_summary=None: f""" 
    ...
    ROLE:
        ...
        5. DO NOT CONCLUDE THE USER REQUEST YOURSELF.
    ...
    TODO:
        - Create a proper todo list for the coder agent.
        - create a detailed todo.
        - Don't Miss out anything
        - clean, clear and easy to follow and easy to understand.
...
- **MODULARITY IS KEY**: Use well-defined functions for complex logic. This makes the code readable and easier to debug.
- Follow best practices (PEP 8).
- Include comprehensive error handling using try/except blocks.
- Save the code whenever you complete the code.

YOUR STRONG AREA:
    - PANDAS & NUMPY: For data manipulation.
    - DUCKDB: Use for SQL queries on Parquet/CSV files (High Performance).
    - PLOTLY: For interactive visualizations.

FOR PLOTS USE PLOTLY ONLY :
    1. Keep it minimal and neat diagram.
    2. Don't use too much color.
    3. Always save them as .html and also display them.
...
STRICT RULES:
    ...
    - CREATE CHARTS ONLY IF REQUESTED BY THE MANAGER.

CODE REQUIREMENTS:
    - Always include necessary imports (e.g., pandas, duckdb, plotly).
    - DUCKDB & SQL: For Parquet files YOU MUST USE DUCKDB. For large CSVs, DuckDB is also preferred.
        - **MANDATORY PATTERN**:
          ```python
          import duckdb
          conn = duckdb.connect()
          ...
          ```
    - FILE PATHS: All datasets are stored in "generated_code/Input/".
    - **SAFE PRINTING**: Always use `.head()` when printing DataFrames.
    - Use meaningful variable names and clear function docstrings.
...
class Agents:
    ...
    def coder_agent_init(self):
        coder_agent = autogen.AssistantAgent(
            name="Coder",
            llm_config=self.get_llm_config(temperature=0.2, max_tokens=5000),
