...
# Note: Using the Minimax config as requested
...
def analyze_request_groupchat(request, metadata_text):
    ...
    try:
        ...
        # Initialize the Agents factory with a single LLM config
        ...
        # Initialize individual agents from the class
        ...
        # Setup Custom Speaker logic (preserved from original script)
        def custom_speaker(last_speaker, groupchat):
            ...
            if last_speaker == coder_agent:
                # Use the class logic to save code
        ...
        # Start conversation
        print(f"[INFO] Starting conversation for request: {request}")
        ...
        # Step 3: Interpret Result (Result Interpreter Agent)
        ...
        # Final Formatting for UI compatibility
        print("--------------------------------------------------------------------------------")
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        req = f"{question}. STRICT INSTRUCTION: The absolute filepath for the input CSV is: '{abs_csv}'. YOU MUST USE THIS EXACT COMPLETE PATH IN ALL YOUR CODE."
    
    analyze_request_groupchat(req, current_metadata)
